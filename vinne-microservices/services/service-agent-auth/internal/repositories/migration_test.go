...
func (s *MigrationTestSuite) TestGooseMigrationsSetup() {
	s.T().Log("✅ Testing Goose Migrations Setup for Agent Auth Service")
	...
	tables := []string{"agents_auth", "retailers_auth", "auth_sessions", "offline_auth_tokens", "password_reset_tokens", "auth_audit_logs"}
	...
	// Test that we can query the agents_auth table structure
	var count int
	err = s.db.QueryRow("SELECT COUNT(*) FROM agents_auth").Scan(&count)
	require.NoError(s.T(), err, "Should be able to query agents_auth table")
	require.Equal(s.T(), 0, count, "agents_auth table should be empty initially")
	s.T().Log("✅ Can query agents_auth table successfully")
...
func (s *MigrationTestSuite) TestMigrationConsistency() {
	...
	testCases := []struct {
		...
		{"agents_auth", "id", "uuid"},
		{"agents_auth", "agent_code", "character varying"},
		{"agents_auth", "phone", "character varying"},
		{"agents_auth", "password_hash", "text"},
		{"retailers_auth", "id", "uuid"},
		{"retailers_auth", "retailer_code", "character varying"},
		{"auth_sessions", "id", "uuid"},
		{"auth_sessions", "user_id", "uuid"},
		{"offline_auth_tokens", "id", "uuid"},
		{"offline_auth_tokens", "user_id", "uuid"},
	}
